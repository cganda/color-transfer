...
public class color_transfer extends PApplet {
...
public void reversibleQuicksort(float[] A, int lo, int hi, int[] indexes) {
  if (hi >= A.length) {
...
public int reversiblePartition(float[] A, int lo, int hi, int[] indexes) {
  float pivot = A[hi];
  int i = lo - 1;
  for (int j = lo; j < hi; j++) {
    if (A[j] < pivot) {
...
public void saveResult(PImage imageToSave) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "color_transfer" };
    if (passedArgs != null) {
